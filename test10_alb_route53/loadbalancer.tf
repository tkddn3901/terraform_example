...
resource "aws_lb_listener" "https" {
  ...
  # 여기에서는 route53.tf를 실행하지 않고 미리 발급받은 인증서의 arn을 사용
...
resource "aws_lb" "web_alb" {
    ...
    subnets = [aws_subnet.public_subnet_1.id, aws_subnet.public_subnet_2.id] # 고가용성을 위해 최소 2개의 public subnet을 제공해야 한다
    tags = { Name = "lecture-alb"}
}

# 3. ALB가 받은 요청을 최종적으로 전달할 대상 그룹
resource "aws_lb_target_group" "web_tg" {
    name = "lecture-tg"
    # 대상 ec2에서 돌아가는 web 서버의 port 번호(변경 가능)
    port = 80
    protocol = "HTTP"
    vpc_id = aws_vpc.main.id
    # 헬스 체크: 로드밸런서가 각 EC2에게 "너 살아있니?"라고 주기적으로 물어보는 설정입니다.
    # 아래의 설정은 health_check를 생략했을 때 적용되는 default 옵션
    health_check {
...
resource "aws_lb_target_group_attachment" "web_attach" {
    # 변수에 정의된 cout 숫자만큼 연결한다
    count = var.ec2_count
    # 위에서 정의한 대상 그룹의 arn을 연결
    target_group_arn = aws_lb_target_group.web_tg.arn
    port = 80
    # count.index를 지정해서 연결
    target_id = aws_instance.my-ec2[count.index].id
}
